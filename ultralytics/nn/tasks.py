...
from .modules.loss_mrl import MatryoshkaDetectionLoss
...
class DetectionModel(BaseModel):
    ...
    def __init__(
        self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True, matryoshka=False
        ...
        self.model, self.save = parse_model(
            deepcopy(self.yaml), ch=ch, verbose=verbose, matryoshka=self.matryoshka
        )  # model, savelist
        ...
        if isinstance(
            ...
            def _forward(x):
                ...
                # For Segment, Pose, OBB, the output is a tuple during training, and we need the first element
                det_preds = preds[0] if isinstance(m, (Segment, Pose, OBB)) else preds

                # For Matryoshka models, the output is a list of lists of tensors.
                # For stride calculation, we only need one of these lists, as spatial dims are the same.
                if (
                    self.matryoshka
                    and isinstance(det_preds, list)
                    and det_preds
                    and isinstance(det_preds[0], list)
                ):
    ...
    def init_criterion(self):
        ...
        if getattr(self, "matryoshka", False):
            return MatryoshkaDetectionLoss(self)
        return (
            E2EDetectLoss(self)
...
def parse_model(d, ch, verbose=True, matryoshka=False):  # model_dict, input_channels(3)
